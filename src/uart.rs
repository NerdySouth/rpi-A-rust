...
pub unsafe fn read_byte() -> u8 {
	...
	let val = (rpi::GET32(aux_mu_io_reg) & 0xff) as u8;
	rpi::dev_barrier();
	val
}
...
pub unsafe fn tx_space_available() -> bool {
	rpi::dev_barrier();
	let tx_stat = rpi::GET32(aux_mu_stat_reg) & 0b10;
	rpi::dev_barrier();
	match tx_stat {
		0 => false,
		_ => true
...
// 		match &mut transmit_bytes(_s.as_bytes()) {
// 			Ok(_) => Ok(()),
// 			Err(UartError::OFF) => Err(core::fmt::Error) 
// 		}
// 	}
// }